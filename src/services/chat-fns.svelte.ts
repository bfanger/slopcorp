...
import parseToolcall, { ToolCallSignature } from "./parseToolcall";
...
export class Conversation {
  ...
  systemPrompt: string;
  tools: Record<string, LanguageModelTool>;
...
Tool description with parameters:\n\n${JSON.stringify(
  ...
  private async processPrompt(
    llm: LanguageModel,
    message: string,
    previousToolCall?: ToolCallSignature,
  ): Promise<void> {
    ...
    try {
      ...
      const call = parseToolcall(toolCalls[0]!.value);
      if (
        previousToolCall &&
        JSON.stringify(call) === JSON.stringify(previousToolCall)
      ) {
        throw new Error("Toolcall is identical");
      }
      const tool = this.tools[call.tool];
      if (!tool) {
        throw new Error(`Tool "${call.tool}" doesn't exists`);
      }
      const answer = await tool.execute(...call.arguments);
      this.messages.push({
        role: "user",
        tool: tool.name,
        content: answer,
